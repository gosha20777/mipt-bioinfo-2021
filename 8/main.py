...
def get_motif_list_from_profile(dna, k, profile):
    return [get_kmer(seq, k, profile) for seq in dna]

def search(dna_list, k, t):
    ...
    while True:
        current_p = get_profile(motif_list)
        motif_list = get_motif_list_from_profile(dna_list, k, current_p)
...
def calculate(input_path: str) -> str:
    with open(input_path, 'r') as file:
        ...
        best_motif_list = search(dna_list, k, t)
        for _ in range(1, 1000):
            new_motif_list = search(dna_list, k, t)
